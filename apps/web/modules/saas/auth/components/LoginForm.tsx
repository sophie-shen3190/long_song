...
export function LoginForm() {
  ...
  const [signinMode, setSigninMode] = useState<'password' | 'magic-link'>('magic-link')

  const [showPassword, setShowPassword] = useState(false)
  const searchParams = useSearchParams()

  const loginWithPasswordMutation = apiClient.auth.loginWithPassword.useMutation()
  const loginWithEmailMutation = apiClient.auth.loginWithEmail.useMutation()

  const form = useForm<FormValues>({ resolver: zodResolver(formSchema) })

  const invitationCode = searchParams.get('invitationCode')
  const redirectTo = invitationCode ? `/team/invitation?code=${invitationCode}` : searchParams.get('redirectTo') ?? '/app'
  ...
  return (
    <div>
      <h1 className='font-extrabold text-3xl md:text-4xl'>{t('auth.login.title')}</h1>
      <p className='mt-4 mb-6 text-muted-foreground'>{t('auth.login.subtitle')}</p>
      ...
      {/* <Form {...form}>
        <form className='space-y-6' onSubmit={form.handleSubmit(onSubmit)}>
          <SigninModeSwitch className='w-full' activeMode={signinMode} onChange={(value) => setSigninMode(value as typeof signinMode)} />
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <div className='relative'>
                      <Input type={showPassword ? 'text' : 'password'} className='pr-10' {...field} autoComplete='current-password' />
                      <button
                        type='button'
          ...
          <Button className='w-full' type='submit' variant='secondary' loading={form.formState.isSubmitting}>
          ...
          <div className='text-center text-sm'>
            ...
            <Link href={`/auth/signup${invitationCode ? `?invitationCode=${invitationCode}&email=${email}` : ''}`}>
