  ...
  return (
    <nav
      ...
      <div className='container'>
        <div className={`flex items-center justify-stretch gap-6 ${isTop ? 'py-8' : 'py-4'} transition-[padding] duration-200`}>
          ...
          <div className='flex flex-1 items-center justify-end gap-3'>
            ...
            <Sheet open={mobileMenuOpen} onOpenChange={(open) => setMobileMenuOpen(open)}>
              <SheetTrigger asChild>
                <Button className='md:hidden' size='icon' variant='outline' aria-label='Menu'>
            ...
            {user ? (
              <Button key='dashboard' className='hidden md:flex' asChild variant='secondary'>
                <Link href='/app'>{t('common.menu.dashboard')}</Link>
              </Button>
            ) : (
              <Button key='login' className='hidden md:flex' asChild variant='secondary'>
